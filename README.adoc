...
Could benefit from additional expressive power ala <<map,`map`>>, <<promisify,`promisify`>>, <<series,`series`>>, or <<tSeries,`tSeries`>>? Use `Zousan-plus`.
...
=== [[map]]`Zousan.map(array,fn) -> newArray`
...
=== [[promisify]]`Zousan.promisify(object, conf) -> promisifiedObject`
...
=== [[series]]`Zousan.series(array | ...args) -> Promise`
...
=== [[tSeries]]`Zousan.tSeries(array | ...args) -> { prom: Promise, res: [resultsArray] }`
